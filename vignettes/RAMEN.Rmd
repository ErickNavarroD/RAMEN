...
```{r}
summarized_methyl_VML = RAMEN::summarizeVML(VML_df = VML_df,
...
```{r}
VML_cis_snps = RAMEN::findCisSNPs(VML_df = VML_df, 
...
VML_cis_snps %>% 
  dplyr::mutate(surrounding_SNPs = case_when( surrounding_SNPs > 3000 ~ 3000,
...
selected_variables %>% 
  dplyr::left_join(VML_cis_snps %>% 
              select(c(VML_index,type)),
            by = "VML_index") %>% 
  dplyr::transmute(VML_index = VML_index,
                   type = type,
...
final_res = lmge_res %>% 
  dplyr::mutate(r2_difference_basal = tot_r_squared - basal_rsquared,
                ...
                model_group = case_when(pass_cutoff_threshold ~ model_group,
                                              TRUE ~ "B")) %>% 
...
selected_variables = fread("path/selected_variables.csv", data.table = FALSE) %>% 
    mutate(selected_genot = str_split(selected_genot, pattern = "\\|"), # fwrite saves lists as strings separated by |, so we need to splut them
