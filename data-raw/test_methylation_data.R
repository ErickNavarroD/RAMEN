...
distribution_betas <- c(
  rbeta(n = sample_size * nrow(test_array_manifest) / 3 * 2, 5, 1), # Methylated distribution - 2 thirds of the distribution
  rbeta(n = sample_size * nrow(test_array_manifest) / 3, 2, 10)
) # Unmethylated distribution - 1 third of the dist
